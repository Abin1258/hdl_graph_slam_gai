...
public:
  ...
  ScanMatchingOdometryNodelet() {
    ...
    if(nh.param<bool>("enable_imu_frontend", false)) {
    ...
    status_pub = nh.advertise<ScanMatchingStatus>("/scan_matching_odometry/status", 8);
...
private:
  ...
  void initialize_params() {
    points_topic = nh.param<std::string>("points_topic", "/velodyne_points");
    odom_frame_id = nh.param<std::string>("scan_matching_odometry/odom_frame_id", "odom");
    robot_odom_frame_id = nh.param<std::string>("scan_matching_odometry/robot_odom_frame_id", "robot_odom");

    // The minimum tranlational distance and rotation angle between keyframes.
    // If this value is zero, frames are always compared with the previous frame
    keyframe_delta_trans = nh.param<double>("scan_matching_odometry/keyframe_delta_trans", 0.25);
    keyframe_delta_angle = nh.param<double>("scan_matching_odometry/keyframe_delta_angle", 0.15);
    keyframe_delta_time = nh.param<double>("scan_matching_odometry/keyframe_delta_time", 1.0);

    // Registration validation by thresholding
    transform_thresholding = nh.param<bool>("scan_matching_odometry/transform_thresholding", false);
    max_acceptable_trans = nh.param<double>("scan_matching_odometry/max_acceptable_trans", 1.0);
    max_acceptable_angle = nh.param<double>("scan_matching_odometry/max_acceptable_angle", 1.0);

    // select a downsample method (VOXELGRID, APPROX_VOXELGRID, NONE)
    std::string downsample_method = nh.param<std::string>("scan_matching_odometry/downsample_method", "VOXELGRID");
    double downsample_resolution = nh.param<double>("scan_matching_odometry/downsample_resolution", 0.1);
  ...
  pcl::Registration<pcl::PointXYZI, pcl::PointXYZI>::Ptr select_registration_method() {
    ...
    std::string registration_method = nh.param<std::string>("scan_matching_odometry/registration_method", "NDT_OMP");
    if(registration_method == "FAST_GICP") {
      ...
      gicp->setNumThreads(nh.param<int>("scan_matching_odometry/reg_num_threads", 0));
      gicp->setTransformationEpsilon(nh.param<double>("scan_matching_odometry/reg_transformation_epsilon", 0.01));
      gicp->setMaximumIterations(nh.param<int>("scan_matching_odometry/reg_maximum_iterations", 64));
      gicp->setMaxCorrespondenceDistance(nh.param<double>("scan_matching_odometry/reg_max_correspondence_distance", 2.5));
      gicp->setCorrespondenceRandomness(nh.param<int>("scan_matching_odometry/reg_correspondence_randomness", 20));
      return gicp;
    }
      ...
      vgicp->setNumThreads(nh.param<int>("scan_matching_odometry/reg_num_threads", 0));
      vgicp->setResolution(nh.param<double>("scan_matching_odometry/reg_resolution", 1.0));
      vgicp->setTransformationEpsilon(nh.param<double>("scan_matching_odometry/reg_transformation_epsilon", 0.01));
      vgicp->setMaximumIterations(nh.param<int>("scan_matching_odometry/reg_maximum_iterations", 64));
      vgicp->setCorrespondenceRandomness(nh.param<int>("scan_matching_odometry/reg_correspondence_randomness", 20));
      return vgicp;
      ...
      icp->setTransformationEpsilon(nh.param<double>("scan_matching_odometry/reg_transformation_epsilon", 0.01));
      icp->setMaximumIterations(nh.param<int>("scan_matching_odometry/reg_maximum_iterations", 64));
      icp->setMaxCorrespondenceDistance(nh.param<double>("scan_matching_odometry/reg_max_correspondence_distance", 2.5));
      icp->setUseReciprocalCorrespondences(nh.param<bool>("scan_matching_odometry/reg_use_reciprocal_correspondences", false));
      return icp;
      ...
      if(registration_method.find("OMP") == std::string::npos) {
        ...
        gicp->setTransformationEpsilon(nh.param<double>("scan_matching_odometry/reg_transformation_epsilon", 0.01));
        gicp->setMaximumIterations(nh.param<int>("scan_matching_odometry/reg_maximum_iterations", 64));
        gicp->setUseReciprocalCorrespondences(nh.param<bool>("scan_matching_odometry/reg_use_reciprocal_correspondences", false));
        gicp->setMaxCorrespondenceDistance(nh.param<double>("scan_matching_odometry/reg_max_correspondence_distance", 2.5));
        gicp->setCorrespondenceRandomness(nh.param<int>("scan_matching_odometry/reg_correspondence_randomness", 20));
        gicp->setMaximumOptimizerIterations(nh.param<int>("scan_matching_odometry/reg_max_optimizer_iterations", 20));
        return gicp;
      } else {
      ...
      double ndt_resolution = nh.param<double>("scan_matching_odometry/reg_resolution", 0.5);
      if(registration_method.find("OMP") == std::string::npos) {
        ...
        ndt->setTransformationEpsilon(nh.param<double>("scan_matching_odometry/reg_transformation_epsilon", 0.01));
        ndt->setMaximumIterations(nh.param<int>("scan_matching_odometry/reg_maximum_iterations", 64));
        ndt->setResolution(ndt_resolution);
        return ndt;
      } else {
        int num_threads = nh.param<int>("scan_matching_odometry/reg_num_threads", 0);
        std::string nn_search_method = nh.param<std::string>("scan_matching_odometry/reg_nn_search_method", "DIRECT7");
  ...
  Eigen::Matrix4f matching(const ros::Time& stamp, const pcl::PointCloud<PointT>::ConstPtr& cloud) {
    ...
    if(nh.param<bool>("enable_imu_frontend", false)) {
    ...
    } else if(nh.param<bool>("scan_matching_odometry/enable_robot_odometry_init_guess", false) && !prev_time.isZero()) {
      ...
      if(transform.stamp_.isZero()) {
        ROS_WARN_STREAM("failed to look up transform between " << cloud->header.frame_id << " and " << robot_odom_frame_id);
      } else {
        msf_source = "odometry";
...
int main(int argc, char** argv) {
    ros::init(argc, argv, "ScanMatchingOdometryNodelet");
    hdl_graph_slam::ScanMatchingOdometryNodelet SMO;
    ROS_INFO("\033[1;32m----> ScanMatchingOdometryNodelet Started.\033[0m");
