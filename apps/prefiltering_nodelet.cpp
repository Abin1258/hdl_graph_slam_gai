...
public:
  ...
  PrefilteringNodelet() {
    ...
    if(nh.param<bool>("prefiltering/deskewing", false)) {
...
private:
  void initialize_params() {
    std::string downsample_method = nh.param<std::string>("prefiltering/downsample_method", "VOXELGRID");
    double downsample_resolution = nh.param<double>("prefiltering/downsample_resolution", 0.1);
    ...
    std::string outlier_removal_method = nh.param<std::string>("prefiltering/outlier_removal_method", "STATISTICAL");
    if(outlier_removal_method == "STATISTICAL") {
      int mean_k = nh.param<int>("prefiltering/statistical_mean_k", 20);
      double stddev_mul_thresh = nh.param<double>("prefiltering/statistical_stddev", 1.0);
      ...
      double radius = nh.param<double>("prefiltering/radius_radius", 0.8);
      int min_neighbors = nh.param<int>("prefiltering/radius_min_neighbors", 2);
    ...
    use_distance_filter = nh.param<bool>("prefiltering/use_distance_filter", true);
    distance_near_thresh = nh.param<double>("prefiltering/distance_near_thresh", 1.0);
    distance_far_thresh = nh.param<double>("prefiltering/distance_far_thresh", 100.0);

    base_link_frame = nh.param<std::string>("prefiltering/base_link_frame", "");
  }
  ...
  pcl::PointCloud<PointT>::ConstPtr deskewing(const pcl::PointCloud<PointT>::ConstPtr& cloud) {
    ...
    double scan_period = nh.param<double>("prefiltering/scan_period", 0.1);
    for(int i = 0; i < cloud->size(); i++) {
...
int main(int argc, char** argv) {
    ros::init(argc, argv, "PrefilteringNodelet");
    hdl_graph_slam::PrefilteringNodelet PF;
    ROS_INFO("\033[1;32m----> PrefilteringNodelet Started.\033[0m");
