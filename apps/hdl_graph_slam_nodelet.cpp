...
public:
  ...
  HdlGraphSlamNodelet() {
    // init parameters
    map_frame_id = nh.param<std::string>("hdl_graph_slam/map_frame_id", "map");
    odom_frame_id = nh.param<std::string>("hdl_graph_slam/odom_frame_id", "odom");
    map_cloud_resolution = nh.param<double>("hdl_graph_slam/map_cloud_resolution", 0.05);
    trans_odom2map.setIdentity();

    max_keyframes_per_update = nh.param<int>("hdl_graph_slam/max_keyframes_per_update", 10);

    //
    anchor_node = nullptr;
    anchor_edge = nullptr;
    floor_plane_node = nullptr;
    graph_slam.reset(new GraphSLAM(nh.param<std::string>("hdl_graph_slam/g2o_solver_type", "lm_var")));
    ...
    gps_time_offset = nh.param<double>("hdl_graph_slam/gps_time_offset", 0.0);
    gps_edge_stddev_xy = nh.param<double>("hdl_graph_slam/gps_edge_stddev_xy", 10000.0);
    gps_edge_stddev_z = nh.param<double>("hdl_graph_slam/gps_edge_stddev_z", 10.0);
    floor_edge_stddev = nh.param<double>("hdl_graph_slam/floor_edge_stddev", 10.0);

    imu_time_offset = nh.param<double>("hdl_graph_slam/imu_time_offset", 0.0);
    enable_imu_orientation = nh.param<bool>("hdl_graph_slam/enable_imu_orientation", false);
    enable_imu_acceleration = nh.param<bool>("hdl_graph_slam/enable_imu_acceleration", false);
    imu_orientation_edge_stddev = nh.param<double>("hdl_graph_slam/imu_orientation_edge_stddev", 0.1);
    imu_acceleration_edge_stddev = nh.param<double>("hdl_graph_slam/imu_acceleration_edge_stddev", 3.0);
    ...
    if(nh.param<bool>("enable_gps", true)) {
      gps_sub = nh.subscribe("/gps/geopoint", 1024, &HdlGraphSlamNodelet::gps_callback, this);
      nmea_sub = nh.subscribe("/gpsimu_driver/nmea_sentence", 1024, &HdlGraphSlamNodelet::nmea_callback, this);
      navsat_sub = nh.subscribe("/gps/navsat", 1024, &HdlGraphSlamNodelet::navsat_callback, this);
    }
    ...
    graph_updated = false;
    double graph_update_interval = nh.param<double>("hdl_graph_slam/graph_update_interval", 3.0);
    double map_cloud_update_interval = nh.param<double>("hdl_graph_slam/map_cloud_update_interval", 10.0);
    optimization_timer = nh.createWallTimer(ros::WallDuration(graph_update_interval), &HdlGraphSlamNodelet::optimization_timer_callback, this);
    map_publish_timer = nh.createWallTimer(ros::WallDuration(map_cloud_update_interval), &HdlGraphSlamNodelet::map_points_publish_timer_callback, this);
  }
...
private:
  ...
  bool flush_keyframe_queue() {
    ...
    for(int i = 0; i < std::min<int>(keyframe_queue.size(), max_keyframes_per_update); i++) {
      ...
      if(keyframes.empty() && new_keyframes.size() == 1) {
        if(nh.param<bool>("hdl_graph_slam/fix_first_node", false)) {
          Eigen::MatrixXd inf = Eigen::MatrixXd::Identity(6, 6);
          std::stringstream sst(nh.param<std::string>("hdl_graph_slam/fix_first_node_stddev", "1 1 1 1 1 1"));
          for(int i = 0; i < 6; i++) {
            double stddev = 1.0;
            sst >> stddev;
            inf(i, i) = 1.0 / stddev;
      ...
      graph_slam->add_robust_kernel(edge, nh.param<std::string>("hdl_graph_slam/odometry_edge_robust_kernel", "NONE"), nh.param<double>("hdl_graph_slam/odometry_edge_robust_kernel_size", 1.0));
    }
  ...
  bool flush_gps_queue() {
    ...
    for(auto& keyframe : keyframes) {
      ...
      graph_slam->add_robust_kernel(edge, nh.param<std::string>("hdl_graph_slam/gps_edge_robust_kernel", "NONE"), nh.param<double>("hdl_graph_slam/gps_edge_robust_kernel_size", 1.0));
  ...
  bool flush_imu_queue() {
    ...
    for(auto& keyframe : keyframes) {
      ...
      if(enable_imu_orientation) {
        ...
        graph_slam->add_robust_kernel(edge, nh.param<std::string>("hdl_graph_slam/imu_orientation_edge_robust_kernel", "NONE"), nh.param<double>("hdl_graph_slam/imu_orientation_edge_robust_kernel_size", 1.0));
      }
      ...
      if(enable_imu_acceleration) {
        ...
        graph_slam->add_robust_kernel(edge, nh.param<std::string>("hdl_graph_slam/imu_acceleration_edge_robust_kernel", "NONE"), nh.param<double>("hdl_graph_slam/imu_acceleration_edge_robust_kernel_size", 1.0));
      }
      updated = true;
    }
  ...
  bool flush_floor_queue() {
    ...
    for(const auto& floor_coeffs : floor_coeffs_queue) {
      ...
      graph_slam->add_robust_kernel(edge, nh.param<std::string>("hdl_graph_slam/floor_edge_robust_kernel", "NONE"), nh.param<double>("hdl_graph_slam/floor_edge_robust_kernel_size", 1.0));
  ...
  void optimization_timer_callback(const ros::WallTimerEvent& event) {
    ...
    for(const auto& loop : loops) {
      ...
      graph_slam->add_robust_kernel(edge, nh.param<std::string>("hdl_graph_slam/loop_closure_edge_robust_kernel", "NONE"), nh.param<double>("hdl_graph_slam/loop_closure_edge_robust_kernel_size", 1.0));
    }
    ...
    if(anchor_node && nh.param<bool>("hdl_graph_slam/fix_first_node_adaptive", true)) {
      Eigen::Isometry3d anchor_target = static_cast<g2o::VertexSE3*>(anchor_edge->vertices()[1])->estimate();
      anchor_node->setEstimate(anchor_target);
    }

    // optimize the pose graph
    int num_iterations = nh.param<int>("hdl_graph_slam/g2o_solver_num_iterations", 1024);
...
int main(int argc, char** argv) {
    ros::init(argc, argv, "HdlGraphSlamNodelet");
    hdl_graph_slam::HdlGraphSlamNodelet HGS;
    ROS_INFO("\033[1;32m----> HdlGraphSlamNodelet Started.\033[0m");
