...
private:
  ...
  void initialize_params() {
    tilt_deg = nh.param<double>("floor_detection/tilt_deg", 0.0);                           // approximate sensor tilt angle [deg]
    sensor_height = nh.param<double>("floor_detection/sensor_height", 2.0);                 // approximate sensor height [m]
    height_clip_range = nh.param<double>("floor_detection/height_clip_range", 1.0);         // points with heights in [sensor_height - height_clip_range, sensor_height + height_clip_range] will be used for floor detection
    floor_pts_thresh = nh.param<int>("floor_detection/floor_pts_thresh", 512);              // minimum number of support points of RANSAC to accept a detected floor plane
    floor_normal_thresh = nh.param<double>("floor_detection/floor_normal_thresh", 10.0);    // verticality check thresold for the detected floor plane [deg]
    use_normal_filtering = nh.param<bool>("floor_detection/use_normal_filtering", true);    // if true, points with "non-"vertical normals will be filtered before RANSAC
    normal_filter_thresh = nh.param<double>("floor_detection/normal_filter_thresh", 20.0);  // "non-"verticality check threshold [deg]
...
int main(int argc, char** argv) {
    ros::init(argc, argv, "FloorDetectionNodelet");
    hdl_graph_slam::FloorDetectionNodelet FD;
    ROS_INFO("\033[1;32m----> FloorDetectionNodelet Started.\033[0m");
